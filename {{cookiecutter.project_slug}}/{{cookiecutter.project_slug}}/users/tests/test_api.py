...
class UserTests(TestCase):
    ...
    def test_login(self):
        ...
        response = self.post('api:user-login', data=data)
        self.response_200(response)
    ...
    def test_list(self):
        with self.login(username=self.user.email):
            response = self.get('api:user-list')
            self.response_200(response)
            self.assertEqual(len(response.json()), 2)

    def test_detail(self):
        with self.login(username=self.user.email):
            response = self.get('api:user-detail', pk=self.user.id)
            self.response_200(response)

    def test_delete(self):
        with self.login(username=self.user.email):
            response = self.delete('api:user-detail', pk=self.user.id)
            self.assertEqual(response.status_code, 204)

    def test_update(self):
        with self.login(username=self.user.email):
            response = self.get('api:user-detail', pk=self.user.id, data={'password': 'new_password'})
            self.response_200(response)
    ...
    def test_create(self):
        with self.login(username=self.user.email):
            ...
            response = self.post('api:user-list', data=data)
    ...
    def test_register(self):
        with self.login(username=self.user.email):
            ...
            response = self.post('api:user-register', data=data)
            self.response_200(response)
    ...
    def test_password_reset(self):
        with self.login(username=self.user.email):
            ...
            response = self.post('api:user-password-reset', data={'email': self.user.email})
            self.response_200(response)

            # email doesn't exist
            response = self.post('api:user-password-reset', data={'email': 'fake-email@gmail.com'})
            self.response_404(response)
    ...
    def test_password_change(self):
        with self.login(username=self.user.email):
            ...
            # token doesn't exist
            response = self.post('api:user-password-change', data={'token': uuid4(), 'password': 'new_password'})
            self.response_404(response)
            ...
            self.user.save()
            response = self.post('api:user-password-change', data={'token': self.user.token, 'password': 'new_password'})
            self.response_200(response)
